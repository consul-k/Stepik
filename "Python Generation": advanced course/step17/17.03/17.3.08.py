...
Напишите программу выводящую все страны, название которых начинается с буквы 'G', численность населения которых больше чем 500 000 человек, 
не меняя их порядок.
...
with open('population.txt') as file:
    ...
    countries = list(filter(lambda x: x[0][0] == 'G' and int(x[-1]) > 500_000, data))
    for c in countries:
        print(c[0])
